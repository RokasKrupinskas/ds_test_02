...
```{r}
namq_10_gdp <- get_eurostat(id="namq_10_gdp", stringsAsFactors = FALSE, filters = list(geo=c("EE","LT", "LV"), na_item="D1",s_adj="SCA", unit="PC_GDP"))
```
...
```{r}
df1 <- aggregate(df$values, list(df$geo), mean)
```
...
ggplot(df, aes(time,values,col=geo))+
        geom_line()+
        scale_x_date(date_labels = (date_format="%Y"),breaks = "1 year")+
        labs(title ="Darbo ir pajamu santykis Lietuvoje, Latvijoje ir Estijoje (2000-2019 metais), procentais", subtitle = "Saltinis: Eurostat (namq_10_gdp)",x= "Laikotarpis",y="Darbo pajamu ir BVP santykis")
```
